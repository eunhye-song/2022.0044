GG1k.LindleySteadyState <- function(A, S, k){
    # simulates a single observation of the steady-state time in system for M/G/1/k queue
    ...
    # sample all service times needed for this replication from the ecdf constructed from S
    Sr_index = ceiling(runif((1+initial_NIS),0,length(S)))
    Sr = S[Sr_index] 
    ...
    if(initial_NIS>0)
    {
        ...
        if(initial_NIS == k) { 
          repeat{
            nextA = A[ceiling(runif(1,0,length(A)))]
            if(prevA + nextA > Sr[1])
            {
              break;
            }else{
